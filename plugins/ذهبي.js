...
export default {
    name: "ذهبي",
    aliases: ["gold"],
    description: "تأثير Individuality (لون وردي/أحمر دافي)",
    category: "tools",
    ...
    async run({ message, sock, reply, react }) {
        ...
        try {
            ...
            // تأثير Individuality الأصلي (وردي/أحمر دافي مع تباين متوسط)
            // تأثير Memory (باهت / حنين) - الجزء الخاص بالتأثير فقط
await execPromise(`ffmpeg -i "${inputFile}" -vf "colorbalance=rs=0.3:gs=0.15:bs=-0.1,eq=saturation=1.2" "${outputFile}" -y`);
            
            await sock.sendMessage(message.key.remoteJid, {
                image: { url: outputFile },
                caption: "*🎨 تـم تـطـبـيـق الـطـابـع الـذهـبـي. 🎨*"
            }, { quoted: message });
