...
export default {
    name: "كلاسيكي",
    aliases: ["كلاسيك"],
    description: "تحويل الصورة إلى تأثير كلاسيكي قديم",
    category: "tools",
    ...
    async run({ message, sock, reply, react }) {
        ...
        try {
            ...
            // تأثير كلاسيكي قديم (أصفر باهت + ناعم)
            await execPromise(`ffmpeg -i "${inputFile}" -vf "colorbalance=rs=0.2:gs=0.05:bs=-0.1,eq=saturation=1.1" "${outputFile}" -y`);
            
            await sock.sendMessage(message.key.remoteJid, {
                image: { url: outputFile },
                caption: "*📻 تـم تـحـويـل الـصـورة إلـى طـابـع كـلاسـيـكـي قـديـم 📻*"
            }, { quoted: message });
