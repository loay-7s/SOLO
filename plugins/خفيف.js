...
export default {
    name: "خفيف",
    aliases: ["light"],
    description: "تحويل الصوت إلى صوت خفيف (نفس السرعة)",
    category: "tools",
    ...
    async run({ message, sock, reply, react }) {
        ...
        if (!mediaMessage) {
            return reply("*❌ ࢪد عـلـى فـويـس بـالأمـر .خفيف*");
        }
        ...
        try {
            ...
            // صوت خفيف نفس السرعة (تغيير التردد وتعويض السرعة)
            await execPromise(`ffmpeg -i "${inputFile}" -af "rubberband=pitch=1.3" "${outputFile}" -y`);
