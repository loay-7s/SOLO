...
export default {
    name: "هندسي",
    aliases: ["رسم", "خطوط", "cartoon"],
    description: "تحويل الصورة إلى تأثير خطوط هندسية كرتونية",
    category: "tools",
    ...
    async run({ message, sock, reply, react }) {
        ...
        try {
            ...
            // تأثير كشف الحواف (خطوط بيضاء على خلفية سوداء)
            await execPromise(`ffmpeg -i "${inputFile}" -vf "edgedetect=low=0.1:high=0.3,negate" "${outputFile}" -y`);
            
            await sock.sendMessage(message.key.remoteJid, {
                image: { url: outputFile },
                caption: "*✏️ تـم تـحـويـل الـصـورة إلـى خـطـوط هـنـدسـيـة 🖌️*"
            }, { quoted: message });
