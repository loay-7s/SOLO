...
export default {
    name: "نيون_بنفسجي",
    aliases: ["purple_neon", "بنفسجي_نيون"],
    description: "تأثير بنفسجي نيون مبهر",
    category: "tools",
    ...
    async run({ message, sock, reply, react }) {
        ...
        try {
            ...
            await execPromise(`ffmpeg -i "${inputFile}" -vf "colorchannelmixer=rr=0.5:rg=0:rb=0.5:gr=0:gg=0:gb=0:br=0.5:bg=0:bb=1.5,eq=contrast=1.3" "${outputFile}" -y`);
            
            await sock.sendMessage(message.key.remoteJid, {
                image: { url: outputFile },
                caption: "*🟣 تـم تـحـويـل الـصـورة إلـى بـنـفـسـجـي نـيـون 🟣*"
            }, { quoted: message });
