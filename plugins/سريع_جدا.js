...
export default {
    name: "سريع_جدا",
    aliases: ["سريعجدا", "fastest"],
    description: "زيادة سرعة الصوت جداً",
    category: "tools",
    ...
    async run({ message, sock, reply, react }) {
        ...
        if (!mediaMessage) {
            return reply("*❌ ࢪد عـلـى فـويـس بـالأمـر .سريع_جدا*");
        }
        ...
        try {
            ...
            await execPromise(`ffmpeg -i "${inputFile}" -filter:a "atempo=2.0" "${outputFile}" -y`);
