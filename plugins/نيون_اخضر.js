...
export default {
    name: "نيون_اخضر",
    aliases: ["green_neon", "اخضر_نيون"],
    description: "تأثير أخضر نيون مبهر",
    category: "tools",
    ...
    async run({ message, sock, reply, react }) {
        ...
        try {
            ...
            await execPromise(`ffmpeg -i "${inputFile}" -vf "colorchannelmixer=rr=0:rg=0:rb=0:gr=0:gg=2:gb=0:br=0:bg=0:bb=0,eq=contrast=1.3" "${outputFile}" -y`);
            
            await sock.sendMessage(message.key.remoteJid, {
                image: { url: outputFile },
                caption: "*🟢 تـم تـحـويـل الـصـورة إلـى أخـضـر نـيـون 🟢*"
            }, { quoted: message });
