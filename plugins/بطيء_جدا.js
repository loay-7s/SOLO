...
export default {
    name: "بطيء_جدا",
    aliases: ["بطيءجدا", "slowest"],
    description: "تخفيف سرعة الصوت جداً",
    category: "tools",
    ...
    async run({ message, sock, reply, react }) {
        ...
        if (!mediaMessage) {
            return reply("*❌ ࢪد عـلـى فـويـس بـالأمـر .بطيء_جدا*");
        }
        ...
        try {
            ...
            await execPromise(`ffmpeg -i "${inputFile}" -filter:a "atempo=0.5" "${outputFile}" -y`);
